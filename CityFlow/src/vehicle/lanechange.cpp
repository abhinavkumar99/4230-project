...
namespace CityFlow{
    ...
    bool LaneChange::planChange() const {
        return (signalSend && signalSend->target && signalSend->target != vehicle->getCurDrivable() && (vehicle->getTurnSoon() || (signalSend->target->getWidth() > vehicle->getWidth()))) || changing;
    }
    ...
    void SimpleLaneChange::makeSignal(double interval) {
        ...
        if (vehicle->getCurDrivable()->isLane()) {
            ...
            // if ((vehicle->getGap() > expectedGap || vehicle->getGap() < 1.5 * vehicle->getLen()) && (curLane->getLaneIndex() != curLane->getBelongRoad()->getLanes().size() - 2)) return;
            ...
            if (curLane->getLaneIndex() < curLane->getBelongRoad()->getLanes().size() - 1){
                if (router.onLastRoad() || router.getNextDrivable(curLane->getOuterLane())) {
                    ...
                    if ((outerEst > curEst + vehicle->getLen()) || (curLane->getOuterLane()->getLaneIndex() == curLane->getBelongRoad()->getLanes().size() - 1)) {
                        signalSend->target = curLane->getOuterLane();
                        vehicle->setTurnSoon(((curLane->getLength() - vehicle->getDistance()) <= 15) && (curLane->getLaneIndex() == curLane->getBelongRoad()->getLanes().size() - 2));
                    }
                }
            }
            ...
            if (curLane->getLaneIndex() > 0){
                if (router.onLastRoad() || router.getNextDrivable(curLane->getInnerLane())) {
                    ...
                    if (innerEst > curEst + vehicle->getLen() && innerEst > outerEst) {
                        ...
                        vehicle->setTurnSoon(((curLane->getLength() - vehicle->getDistance()) <= 15) && (curLane->getLaneIndex() == 1));
                    }
                }
            }
